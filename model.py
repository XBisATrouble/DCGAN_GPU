...
class DCGAN(object):
    ...
    def discriminator(self, image, y=None, reuse=False, feature=False):
        print("image:", image)
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                ...
                if not feature:
                    ...
                    # 池化层入参只有一个参数，即为kernel的边长
                    max_pool1 = tf.nn.max_pool2d(h0,
                                                 ksize=[1, int((h0.get_shape()[2].value / 4)),
                                                        int((h0.get_shape()[2].value / 4)), 1],
                                                 strides=[1, 1, 1, 1],
                                                 padding='VALID')
                    max_pool2 = tf.nn.max_pool2d(h1,
                                                 ksize=[1, int((h1.get_shape()[2].value / 4)),
                                                        int((h1.get_shape()[2].value / 4)), 1],
                                                 strides=[1, 1, 1, 1],
                                                 padding='VALID')
                    max_pool3 = tf.nn.max_pool2d(h2,
                                                 ksize=[1, int((h2.get_shape()[2].value / 4)),
                                                        int((h2.get_shape()[2].value / 4)), 1],
                                                 strides=[1, 1, 1, 1],
                                                 padding='VALID')

                    vector1 = tf.reshape(max_pool1, shape=[tf.shape(image)[0], -1])
                    vector2 = tf.reshape(max_pool2, shape=[tf.shape(image)[0], -1])
                    vector3 = tf.reshape(max_pool3, shape=[tf.shape(image)[0], -1])

                    return tf.concat([vector1, vector2, vector3], 1)
            else:
